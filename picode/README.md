...
**ECGprocess:** Responsible for reading ECG data from the MCP3008 ADC which in turn is reading from the analogue AD8232 sensor. The raw data is then filtered in real time and pushed to queues to be locally displayed and pushed to the database. This is in a completely seperate process so that it is only run if the ECG 3 lead connector is plug
**Respirationprocess:**
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwOTgzNzIyNDgsMTQxNDg0NDQ4MiwtOT
