...
def calculate_HR(IR, lowcut, highcut, fs=100, order=5):
    '''
...
##    plt.figure()
##    plt.plot(dataset_HR)
##    plt.title("("+netid+") Bandpass-Filtered 5th-Order IR Signal for HR\n ("+activity_name+")")
...
##    plt.figure()
##    plt.title("("+netid+") Detected Peaks in IR Signal for Heart Rate ("+activity_name+")")
##    #plt.xlim(2600,2800)
##    plt.ylim(-600,600)
...
##    plt.scatter(newpeaklist, newybeat, color='red') #Plot detected peaks
    ...
    #plt.figure()
    #plt.plot(heart_rate)
    #plt.title("("+netid+") Plot of Heart Rate ("+activity_name+")")
    #plt.xlabel("Peaks")
    #plt.ylabel("HR (beats/min)")
    #plt.savefig(netid+'_figures/'+netid+'_'+activity_name+'_HR.png')
    
    print("the mean of heart rate is {2:.2f} beats/min.\n".format(np.mean(heart_rate)))
    #plt.show()
    return (heart_rate)


def calculate_RR(IR, lowcut, highcut, fs=50, order=5):
    '''
