...
ecg_line, =ax_ecg.plot(xs, y_ecg, color="#59eaed")
ax_ecg.set_ylim([-2,2])
# PPG Graph pane
...
ppg_line, =ax_ppg.plot(xs, y_ppg, color="#f51a18")
ax_ppg.set_ylim([-2,2])
...
rr_line, =ax_resp.plot(xs, y_rr, color="#3cd82c")
ax_resp.set_ylim([-2,2])
# Report issue button
...
def animateecg(i,ys):
    ...
    ys.append(math.sin(x_ecg/2*math.pi)) 
    #y_ppg.append(x)
    #y_resp.append(x**3)
...
def animaterr(i,ys):
    ...
    # Read data into arrays
    #xar.append(dt.datetime.utcnow())
    ys.append(math.sin(x_resp/2*math.pi)) 
    #y_ppg.append(x)
    #y_resp.append(x**3)
    ...
    # remove - just for RT testing
    x_resp += 1
    if (x_resp/(2*math.pi))==1:
        x_resp=0
    #print("helooo")
    rr_line.set_ydata(ys)
    #print("wassp")
    #ppg_line.set_ydata(y_ppg)
    #rr_line.set_ydata(y_resp)
    return rr_line,
...
ani1 = animation.FuncAnimation(fig_ecg, animateecg, fargs = (y_ecg,), interval=20,blit=True) # animate graph every 20 ms
ani2 = animation.FuncAnimation(fig_ppg, animateppg, fargs = (y_ppg,), interval=20,blit=True) # animate graph every 20 ms
ani3 = animation.FuncAnimation(fig_resp, animaterr, fargs = (y_rr,), interval=20,blit=True) # animate graph every 20 ms
